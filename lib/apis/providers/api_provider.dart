...
import 'package:my_ndu/constants/urls.dart';
import 'package:my_ndu/helpers/exceptions.dart';
...
class ApiProvider {
  ...
  ///Get student model
  Future<ResponseData> fetchStudentModel(String token) async {
    final response = await _catchAsyncApiError(
      endPoint: AppUrls.fetchStudentModelEndpoint,
      method: 'GET',
  ...
  Future<ResponseData> checkStudent(String token) async {
    final response = await _catchAsyncApiError(
      endPoint: AppUrls.checkStudentEndpoint,
      method: 'GET',
      feature: 'Check Student Profile',
      headers: {'authorization': 'Bearer $token'},
    );
